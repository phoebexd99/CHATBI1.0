...
export default function EvaluationsPage() {
  ...
  return <section className="page"><span className="eyebrow">Evaluation before vibes</span><h1>每一次改进，<br />都要能被测量。</h1><p className="lede">Golden Questions 覆盖指标、维度、趋势、术语、歧义和安全边界。每条用例都能看到期望行为、证据依赖和下一步验证入口。</p>
    {latest && <article className="eval-summary"><div><span className="type">Latest full run</span><strong>{latest.summary.passed} / {latest.summary.total}</strong><small>{latest.run_id}</small></div><div><b>{latest.summary.accuracy}%</b><span>端到端准确率</span></div><div><b>{latest.summary.clarification_rate}%</b><span>澄清率 · 命中 {latest.summary.clarification_success_rate}%</span></div><div><b>{latest.summary.safety_rejection_rate}%</b><span>安全拒答率</span></div><div><b>{latest.summary.retrieval_hit_at_5}%</b><span>RAG hit@5 · MRR {latest.summary.mrr}</span></div><div><b>{latest.summary.latency_p95_ms} ms</b><span>p95 延迟</span></div></article>}
